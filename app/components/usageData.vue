<template>
    <div
        class="w-80 p-4 h-fit bg-yu-background-50 rounded-lg border border-yu-border-200 flex flex-col gap-3"
    >
        <h1 class="text-xl font-bold">
            Usage
            <span class="text-base text-text-200"
                >for {{ currentDate.month }}, {{ currentDate.year }}</span
            >
        </h1>
        <Divider />
...
function calculateUsageByField(typedLogs: Log[], field: keyof Log) {
    return typedLogs
        .reduce(
            (acc, log) => {
                ...
                if (index === -1) acc.push({ name: value, count: 1 });
